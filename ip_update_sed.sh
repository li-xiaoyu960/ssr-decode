...
ss1new=$(sed -n '1p' ip_update_latest.json)
echo "s1下载新节点$ss1new"

ss1old=$(sed -n '11p' jms.yaml)
echo "s1现使用节点$ss1old"
...
#第二个节点：在jms.yaml文件中取得旧IP，在ip_update_latest.json文件中取得新IP，对比并替换
ss2new=$(sed -n '2p' ip_update_latest.json)
echo "s2下载新节点$ss2new"

ss2old=$(sed -n '17p' jms.yaml)
echo "s2现使用节点$ss2old"
...
#第三个节点：在jms.yaml文件中取得旧IP，在ip_update_latest.json文件中取得新IP，对比并替换
vmess3new=$(sed -n '3p' ip_update_latest.json)
echo "s3下载新节点$vmess3new"

vmess3old=$(sed -n '23p' jms.yaml)
...
#第四个节点：在jms.yaml文件中取得旧IP，在ip_update_latest.json文件中取得新IP，对比并替换
vmess4new=$(sed -n '4p' ip_update_latest.json)
echo "s4下载新节点$vmess4new"

vmess4old=$(sed -n '33p' jms.yaml)
...
#第五个节点：在jms.yaml文件中取得旧IP，在ip_update_latest.json文件中取得新IP，对比并替换
vmess5new=$(sed -n '5p' ip_update_latest.json)
echo "s5下载新节点$vmess5new"

vmess5old=$(sed -n '44p' jms.yaml)
...
#第六个节点：在jms.yaml文件中取得旧IP，在ip_update_latest.json文件中取得新IP，对比并替换

vmess801new=$(sed -n '6p' ip_update_latest.json)
echo "s801下载新节点$vmess801new"

vmess801old=$(sed -n '54p' jms.yaml)
