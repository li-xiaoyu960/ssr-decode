...
ss1old=$(sed -n '4p' /root/.config/clash/proxy-providers/jms.yaml)
echo "s1现使用节点$ss1old"

if [ -n "$ss1new" ] && [ "$ss1new" = "$ss1old" ]; then
	echo "s1节点IP相同，未作更换"
elif [ -n "$ss1new" ] && [ "$ss1new" != "$ss1old" ]; then
	sed -i "4 s/$ss1old/$ss1new/" /root/.config/clash/proxy-providers/jms.yaml
	ss1update=$(sed -n '4p' /root/.config/clash/proxy-providers/jms.yaml)
...
ss2old=$(sed -n '11p' /root/.config/clash/proxy-providers/jms.yaml)
echo "s2现使用节点$ss2old"

if [ -n "$ss2new" ] && [ "$ss2new" = "$ss2old" ]; then
        echo "s2节点IP相同，未作更换"
elif [ -n "$ss2new" ] && [ "$ss2new" != "$ss2old" ]; then
        sed -i "11 s/$ss2old/$ss2new/" /root/.config/clash/proxy-providers/jms.yaml
        ss2update=$(sed -n '11p' /root/.config/clash/proxy-providers/jms.yaml)
        changecount=`expr $changecount + 1`
...
vmess3old=$(sed -n '18p' /root/.config/clash/proxy-providers/jms.yaml)
echo "s3现使用节点$vmess3old"

if [ -n "$vmess3new" ] && [ "$vmess3new" = "$vmess3old" ]; then
        echo "s3节点IP相同，未作更换"
elif [ -n "$vmess3new" ] && [ "$vmess3new" != "$vmess3old" ]; then
        sed -i "18 s/$vmess3old/$vmess3new/" /root/.config/clash/proxy-providers/jms.yaml
        vmess3update=$(sed -n '18p' /root/.config/clash/proxy-providers/jms.yaml)
        changecount=`expr $changecount + 1`
...
vmess4old=$(sed -n '27p' /root/.config/clash/proxy-providers/jms.yaml)
echo "s4现使用节点$vmess4old"

if [ -n "$vmess4new" ] && [ "$vmess4new" = "$vmess4old" ]; then
        echo "s4节点IP相同，未作更换"
elif [ -n "$vmess4new" ] && [ "$vmess4new" != "$vmess4old" ]; then
        sed -i "27 s/$vmess4old/$vmess4new/" /root/.config/clash/proxy-providers/jms.yaml
        vmess4update=$(sed -n '27p' /root/.config/clash/proxy-providers/jms.yaml)
        changecount=`expr $changecount + 1`
...
vmess5old=$(sed -n '36p' /root/.config/clash/proxy-providers/jms.yaml)
echo "s5现使用节点$vmess5old"

if [ -n "$vmess5new" ] && [ "$vmess5new" = "$vmess5old" ]; then
        echo "s5节点IP相同，未作更换"
elif [ -n "$vmess5new" ] && [ "$vmess5new" != "$vmess5old" ]; then
        sed -i "36 s/$vmess5old/$vmess5new/" /root/.config/clash/proxy-providers/jms.yaml
        vmess5update=$(sed -n '36p' /root/.config/clash/proxy-providers/jms.yaml)
        changecount=`expr $changecount + 1`
...
vmess801old=$(sed -n '45p' /root/.config/clash/proxy-providers/jms.yaml)
...
if [ -n "$vmess801new" ] && [ "$vmess801new" = "$vmess801old" ]; then
        ...
        sed -i "45 s/$vmess801old/$vmess801new/" /root/.config/clash/proxy-providers/jms.yaml
        vmess801update=$(sed -n '45p' /root/.config/clash/proxy-providers/jms.yaml)
        changecount=`expr $changecount + 1`
